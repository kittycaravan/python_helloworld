...
def get_temp(): 
    # target_num = int(request.args.get('n')) #파라미터로 넘어온 값을 빼서 변수에 대입함.
    target_num = str(request.args.get('n')) #파라미터로 넘어온 값을 빼서 변수에 대입함.
    print("=======",target_num)

    #### 5.예측을 위해 올려둔 이미지 파일 읽기
    # img = cv2.imread('7.bmp') # 여기 바꿔가면서 테스트하자. 7 정답
    img = cv2.imread(target_num+'.bmp') # 여기 바꿔가면서 테스트하자. 7 정답
